...
def batchify_data(x_data, y_data, batch_size):
    ...
    for i in range(0, N, batch_size):
        batches.append(
            {
                'x': torch.tensor(x_data[i:i+batch_size], dtype=torch.float32).to(device),
                'y': torch.tensor(y_data[i:i+batch_size], dtype=torch.long).to(device),
            }
        )
    return batches
...
def compute_accuracy(predictions, y):
    ...
    predictions, y = tensor2array(predictions), tensor2array(y)
